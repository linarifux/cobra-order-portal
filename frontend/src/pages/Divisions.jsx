...
export default function Divisions() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] bg-blue-400/20 rounded-full mix-blend-multiply filter blur-[100px] pointer-events-none"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-indigo-400/20 rounded-full mix-blend-multiply filter blur-[100px] pointer-events-none"></div>

      <div className="w-full max-w-2xl space-y-8 relative z-10 animate-in fade-in zoom-in-95 duration-700">
        ...
        {divisionList.length > 0 ? (
            ...
            {divisionList.map((division, idx) => {
              ...
              return (
                <div
                  ...
                  className={`relative flex flex-col justify-between p-6 rounded-[2rem] border transition-all duration-300 cursor-pointer group bg-white/70 backdrop-blur-3xl backdrop-saturate-150 ${
                    isCurrentlyActive
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div className={`flex h-12 w-12 items-center justify-center rounded-xl border transition-colors shadow-inner ${
                        isCurrentlyActive 
                      ...
                      {isCurrentlyActive ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-emerald-50 text-emerald-700 text-[10px] font-bold uppercase tracking-wider border border-emerald-200/50 shadow-sm">
                          <CheckCircle2 className="h-3 w-3" /> Active Workspace
                        </span>
                      ) : (
                        <span className="opacity-0 group-hover:opacity-100 transition-opacity flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 text-slate-600">
                    ...
                    {/* Title Profile */}
                    <h3 className="text-lg font-extrabold text-gray-900 tracking-tight transition-colors group-hover:text-blue-600">
                      {currentName}
                    </h3>
                    <p className="mt-1.5 font-mono text-[11px] font-bold text-gray-400 tracking-tight truncate">
                      ID: {currentId}
                    </p>
                  </div>

                  <div className="mt-8 pt-4 border-t border-gray-100/60 flex items-center justify-between text-xs font-semibold text-gray-400 group-hover:text-gray-500">
                    <span>Authorized Portal</span>
                    <span className="uppercase tracking-wider font-bold text-gray-600">{user?.portal || 'Order'}</span>
                  </div>
                </div>
              );
            })}
          ...
          <div className="rounded-[2.5rem] bg-white/60 backdrop-blur-3xl p-10 border border-white/80 text-center shadow-lg shadow-black/5">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-red-50 text-red-500 border border-red-100 shadow-inner mb-4">
              <ShieldAlert className="h-6 w-6" />
            </div>
            <h3 className="text-lg font-bold text-gray-900">No Divisions Allocated</h3>
